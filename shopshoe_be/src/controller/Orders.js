...
export const createOrder = async (req, res, next) => {
  try {
    ...
    return res.status(201).json({ message: "Đơn hàng đã được tạo.", newOrder });
  } catch (error) {
...
export const checkAndUpdatePaymentStatus = async (req, res) => {
  try {
    ...
    console.error("Lỗi khi kiểm tra và cập nhật trạng thái:", error.message);
    res.status(500).json({
...
export const getOrderDetail = async (req, res, next) => {
  try {
    ...
    const order = await Order.findById(orderId).populate("products.product"); // Tìm đơn hàng theo ID và populate thông tin sản phẩm
    if (!order) {
      return res.status(404).json({ message: "Đơn hàng không tồn tại." });
    }
...
export const updateStatusCancel = async (req, res, next) => {
  ...
  try {
    ...
    if (order.status === "Shipping") {
      return res
        .status(400)
        .json({ message: "Cannot cancel an order that is in Shipping" });
    }
    ...
    for (const item of order.products) {
      ...
      // Tìm sản phẩm
      const productDoc = await Products.findById(product).session(session);
      ...
      if (!sizeStockItem) {
        throw new Error(`Size with ID ${size} not found in product ${product}`);
      }

...
export const createRefund = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Lỗi máy chủ. Không thể tạo yêu cầu hoàn tiền." });
    }
  });
};
...
export const getAllRefunds = async (req, res) => {
  try {
    const refunds = await Refund.find().populate("orderId").populate("userId");
...
export const updateRefundStatus = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Lỗi máy chủ. Không thể cập nhật trạng thái yêu cầu hoàn tiền.",
    });
  }
};
...
export const deleteRefund = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Yêu cầu hoàn tiền đã được xóa thành công." });
  } catch (error) {
    console.error("Lỗi khi xóa yêu cầu hoàn tiền:", error);
    res
      .status(500)
      .json({ message: "Lỗi máy chủ. Không thể xóa yêu cầu hoàn tiền." });
  }
};

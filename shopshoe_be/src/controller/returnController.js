...
export const createReturn = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      const { orderId, userId, userName, userPhone, reason, status } = req.body;
      const images = req.files["images"]
        ? req.files["images"].map((file) => `/uploads/returns/${file.filename}`)
        : [];
      const videos = req.files["videos"]
        ? req.files["videos"].map((file) => `/uploads/returns/${file.filename}`)
        : [];
      ...
      res
        .status(500)
        .json({ message: "Lỗi máy chủ. Không thể tạo yêu cầu hoàn trả." });
    }
  });
};
// lấy danh sáchsách
export const getAllReturns = async (req, res) => {
  try {
    const returns = await Return.find().populate("orderId").populate("userId");
...
export const updateReturnStatus = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Lỗi máy chủ. Không thể cập nhật trạng thái yêu cầu hoàn trả.",
    });
  }
};
  ...
  try {
    ...
    if (returnRequest.images && returnRequest.images.length > 0) {
      returnRequest.images.forEach((imagePath) => {
        const fullPath = path.join(__dirname, "../uploads/returns", imagePath);
    ...
    if (returnRequest.videos && returnRequest.videos.length > 0) {
      returnRequest.videos.forEach((videoPath) => {
        const fullPath = path.join(__dirname, "../uploads/returns", videoPath);
        if (fs.existsSync(fullPath)) {
          fs.unlinkSync(fullPath);
        }
      });
    ...
    res
      .status(200)
      .json({ message: "Yêu cầu hoàn trả đã được xóa thành công." });
  } catch (error) {
    console.error("Lỗi khi xóa yêu cầu hoàn trả:", error);
    res
      .status(500)
      .json({ message: "Lỗi máy chủ. Không thể xóa yêu cầu hoàn trả." });
  }
};

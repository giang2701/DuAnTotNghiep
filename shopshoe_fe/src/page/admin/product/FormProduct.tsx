    ...
    if (id) {
      (async () => {
        try {
          ...
          if (existingData.imgCategory && existingData.imgCategory.length > 0) {
            setImgCategoryURLs(existingData.imgCategory);
          }
          // Đảm bảo sizeStock được định dạng đúng
          const updatedSizeStock = existingData.sizeStock.map((item: any) => ({
            ...
            <div className="col-6">
              ...
              <div className="form-group mb-2">
                ...
                <div className="custom-multiple-upload">
                  <input
                    ...
                    multiple
                    className={`file-input ${
                      errors.imgCategory ? "is-invalid" : ""
                    }`}
                  />
                  {imgCategoryURLs.length === 0 && (
                    <span className="upload-placeholder">Chọn ảnh</span>
                  )}
              ...
              <div>
                ...
                {fields.map((field, index) => (
                  <div key={field.id} className="size-stock-row d-flex mb-2">
                    <select
                      {...register(`sizeStock.${index}.size._id`)}
                      style={{
                        width: "100px",
                        height: "25px",
                      }}
                    ...
                    <input
                      type="number"
                      {...register(`sizeStock.${index}.stock`)}
                    ...
                    <input
                      type="number"
                      {...register(`sizeStock.${index}.price`)}
                      placeholder="Giá"
                      className={`form-control ${

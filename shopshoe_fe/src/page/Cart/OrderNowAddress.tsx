...
const CheckoutNow = () => {
    ...
    const totalPriceNow = productData.price * productData.quantity;

    const productDataNow = {
        product: productData.id,
        quantity: productData.quantity,
        size: productData.size.id,
    };
    // form sửa thông tin user từ local storage
    ...
    useEffect(() => {
        (async () => {
            const userAddress = userLocal ? JSON.parse(userLocal) : null;
            reset(userAddress);
        })();
    }, [reset]);
    const handleEditAddress = async (data: any) => {
        ...
        try {
            const dataAddress = await instance.put(
                `/user/address/${userAddress._id}`,
                data
            );
            localStorage.setItem("user", JSON.stringify(dataAddress.data.data));
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const handPayment = async () => {
        try {
            ...
            if (selectedValue === "MOMO") {
                const response = await axios.post(
                    "http://localhost:8000/api/orders/payWithMoMoDetail",
                    orderData
                );
                ...
                response = await axios.post(
                    "http://localhost:8000/api/orders/detail",
                    orderData
                );
    ...
    return (
        <>
            <div className="container">
                ...
                <form action="" onSubmit={handleSubmit(handPayment)}>
                    <Grid container spacing={4} justifyContent="center">
                        ...
                        <Grid item xs={12} md={6}>
                            ...
                            >
                                ...
                                >
                                    ...
                                    Thông tin thẻ tín dụng sẽ không bao giờ được
                                    lưu lại.
                                </Typography>
                        ...
                        <Grid item xs={12} md={6}>
                            ...
                            <Box display="flex" justifyContent="center">
                                <button className="btn_checkout mt-5">
                                    Thanh Toán
                                </button>
                            </Box>
                        </Grid>
                    ...
                    {vouchers.map((voucher) => {
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    opacity:
                                        isExpired || isBelowMinPrice ? 0.5 : 1, // Làm mờ nếu hết hạn hoặc dưới giá trị tối thiểu
                                    cursor:
                                        isExpired || isBelowMinPrice
                                            ? "not-allowed"
                                            : "pointer", // Không cho click
            ...
            {/* form sửa địa chỉ */}
            <div
                className={`overlay_userAddress ${
                    editAddress ? "d-block" : "d-none"
                } `}
                onClick={() => setEditAddress(false)}
            ></div>
            <div
                className={`box_userAddress ${
                    editAddress ? "d-block" : "d-none"
                }`}
            >
                <form action="" onSubmit={handleSubmit(handleEditAddress)}>
                    <p className="fs-3">Thay Đổi Thông Tin </p>
                    <div className="mb-3">
                    ...
                    <div className="mb-3 d-flex justify-content-end">
                        <span
                            className="btn btn-primary me-3 d-inline-block"
                            style={{
                                borderRadius: "5px",
                            }}
                            onClick={() => setEditAddress(false)}
